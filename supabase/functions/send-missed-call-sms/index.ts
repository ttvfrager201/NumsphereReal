...
Deno.serve(async (req) => {
    ...
    try {
        ...
        let messageBody = config.sms_message_template ||
        ...
        let toNumber = missedCall.phone_number.replace(/\s/g, "").replace(/[()]/g, "").replace(/-/g, "");
        ...
        if (!twilioRes.ok) {
            ...
            throw new Error(
                twilioResult.message || "Failed to send SMS via Twilio"
            );
        }
        ...
        await supabase
            .from("missed_calls")
            .update({ status: "texted", updated_at: new Date().toISOString() })
            .eq("id", missed_call_id);
