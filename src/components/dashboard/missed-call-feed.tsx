...
export function MissedCallFeed({ calls, onSendLink, onMarkHandled }: MissedCallFeedProps) {
  ...
  return (
    <Card className="col-span-12 lg:col-span-7 h-[600px] flex flex-col bg-card/50 border-border/50 backdrop-blur-sm">
      <div className="p-6 border-b border-border/50">
        <h3 className="font-display font-bold text-xl text-foreground">Missed Call Feed</h3>
          ...
          {calls.length === 0 ? (
            ...
            calls.map((call) => (
              <motion.div
                ...
                layout
                className={cn(
                  "group relative bg-card p-5 rounded-lg border transition-all hover:shadow-md hover:-translate-y-[2px]",
                  call.status === "new" && "border-l-4 border-l-amber-500",
                  call.status === "texted" && "border-l-4 border-l-teal-600",
                  call.status === "booked" && "border-l-4 border-l-gray-400",
                  call.status === "handled" && "border-l-4 border-l-gray-300 opacity-75"
                )}
              >
                <div className="flex items-start justify-between gap-4">
                  ...
                  <div className="flex items-center gap-2">
                    {call.status === "new" && (
                      <>
                        ...
                        <Button
                          size="sm"
                          className={cn(
                            "h-9 px-4 font-medium bg-teal-700 hover:bg-teal-800 text-white transition-all duration-300",
                            sendingId === call.id && "w-12 px-0 bg-green-600 hover:bg-green-700"
                          )}
                    ...
                    {call.status === "texted" && (
                      <span className="text-xs font-medium px-3 py-1 bg-teal-100 text-teal-800 rounded-full flex items-center gap-1">
                        <Check className="w-3 h-3" /> Link Sent
                      </span>
                    )}
                    
                    {call.status === "booked" && (
                      <span className="text-xs font-medium px-3 py-1 bg-green-100 text-green-800 rounded-full flex items-center gap-1">
