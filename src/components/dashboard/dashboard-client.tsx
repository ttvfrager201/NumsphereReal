...
export function DashboardClient({ missedCalls, bookings, settings, stats }: DashboardClientProps) {
  ...
  const [userData, setUserData] = useState<{ email: string; name: string; avatarUrl: string | null } | null>(null);
  ...
  return (
    <div className="min-h-screen bg-black text-white selection:bg-white/20 flex">
      ...
      <Link href="/" className="fixed top-0 left-0 z-50 p-4 flex items-center gap-2">
        <div className="w-8 h-8 bg-white rounded-full flex items-center justify-center">
          <span className="text-black font-bold text-lg">N</span>
        </div>
        <span className="text-xl font-bold text-white tracking-tighter">NumSphere</span>
      </Link>
            ...
            return (
              <button
                ...
                className={cn(
                  ...
                  sidebarCollapsed ? "justify-center px-3 py-3" : "px-4 py-3 text-left",
                  activeNav === item.id
          ...
          <div className="mt-auto pt-6 border-t border-white/10 flex flex-col gap-2">
            ...
            <div className="flex flex-col gap-1">
              <Button
                ...
                className={cn(
                  "text-gray-400 hover:bg-white/5 hover:text-white",
                  sidebarCollapsed ? "justify-center px-3" : "justify-start gap-3"
                )}
              >
        ...
        <div className="max-w-5xl mx-auto">
            ...
            <div className="space-y-8">
              ...
              <div className="grid gap-6 sm:grid-cols-2 lg:grid-cols-3">
                <div className="min-h-[180px] rounded-xl border border-white/10 bg-white/5 p-6 flex flex-col justify-between">
                  <p className="text-gray-500 uppercase tracking-wider text-xs">Missed Calls Today</p>
                  <p className="text-3xl font-bold text-white">{stats.missedCallsToday}</p>
                </div>
                <div className="min-h-[180px] rounded-xl border border-white/10 bg-white/5 p-6 flex flex-col justify-between">
                  <p className="text-gray-500 uppercase tracking-wider text-xs">Bookings Today</p>
                  <p className="text-3xl font-bold text-white">{stats.bookingsToday}</p>
                </div>
                <div className="min-h-[180px] rounded-xl border border-white/10 bg-white/5 p-6 flex flex-col justify-between">
                  <p className="text-gray-500 uppercase tracking-wider text-xs">Revenue Captured</p>
                  <p className="text-3xl font-bold text-white">${stats.revenueCaptured}</p>
                </div>
              </div>
          ...
          {activeNav === "missed-calls" && (
            <div className="space-y-6">
              <h1 className="text-2xl font-bold tracking-tight text-white">Missed Call Text-Back</h1>
              <p className="text-gray-400">Automatically text back when you miss a call.</p>
          ...
          {activeNav === "dashboard" && (
            <div className="space-y-6">
              <h1 className="text-2xl font-bold tracking-tight text-white">Basic Dashboard</h1>
              ...
              <div className="grid gap-6 sm:grid-cols-2">
                <div className="min-h-[300px] rounded-xl border border-white/10 bg-white/5 p-8 flex items-center justify-center">
                  <p className="text-gray-500 uppercase tracking-wider text-sm">Calls</p>
                </div>
                <div className="min-h-[300px] rounded-xl border border-white/10 bg-white/5 p-8 flex items-center justify-center">
                  <p className="text-gray-500 uppercase tracking-wider text-sm">Appointments</p>
                </div>
              </div>
            </div>
          )}
